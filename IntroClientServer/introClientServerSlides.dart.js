...
//  ********** Library introClientServerSlides **************
// ********** Code for introClientServerSlides **************
function introClientServerSlides() {
  this.PERMANENT_URL_PREFIX = "http://html5slides.googlecode.com/svn/trunk/";
  this.curSlide = (0);
}
introClientServerSlides.prototype.addGeneralStyle = function() {
  ...
  html_get$document().get$body().get$nodes().add(el);
  ...
  html_get$document().query("head").get$nodes().add(el);
  el = ElementWrappingImplementation.ElementWrappingImplementation$tag$factory("meta");
  el.set$name("apple-mobile-web-app-capable");
  el.set$content("yes");
  html_get$document().query("head").get$nodes().add(el);
}
introClientServerSlides.prototype.setupInteraction = function() {
  var $this = this; // closure support
  var el = ElementWrappingImplementation.ElementWrappingImplementation$tag$factory("div");
  el.get$classes().add("slide-area");
  el.set$id("prev-slide-area");
  el.get$on().get$click().add$2((function (event) {
    $this.prevSlide();
  })
  , false);
  html_get$document().query("section.slides").get$nodes().add(el);
...
introClientServerSlides.prototype.getSlideEl = function(no) {
  if (($lt(no, (0))) || ($gte(no, this.slideEls.get$length()))) {
    return null;
  }
  else {
...
introClientServerSlides.prototype.updateSlideClass = function(slideNo, className) {
  var el = this.getSlideEl(slideNo);
  if ($eq(el)) {
    return;
  }
  if ((typeof(className) == 'string')) {
    el.get$classes().add(className);
  }
  var $$list = this.SLIDE_CLASSES;
  for (var $$i = $$list.iterator(); $$i.hasNext(); ) {
    var i = $$i.next();
    if ($ne(className, i)) {
      el.get$classes().remove(i);
    }
  }
}
introClientServerSlides.prototype.triggerLeaveEvent = function(no) {
  var el = this.getSlideEl(no);
  if ($eq(el)) {
    return;
  }
}
introClientServerSlides.prototype.triggerEnterEvent = function(no) {
  var el = this.getSlideEl(no);
  if ($eq(el)) {
    return;
  }
}
introClientServerSlides.prototype.disableSlideFrames = function(no) {
  var el = this.getSlideEl(no);
  if ($eq(el)) {
    return;
  }
}
introClientServerSlides.prototype.enableSlideFrames = function(no) {
  var el = this.getSlideEl(no);
  if ($eq(el)) {
    return;
  }
}
introClientServerSlides.prototype.updateHash = function() {
  html_get$window().get$location().replace("#" + (this.curSlide + (1)));
}
introClientServerSlides.prototype.updateSlides = function() {
  var $this = this; // closure support
  for (var i = (0);
   $lt(i, this.slideEls.get$length()); i = $add(i, (1))) {
    switch (i) {
...
introClientServerSlides.prototype.buildNextItem = function() {
  var toBuild = this.slideEls.$index(this.curSlide).queryAll(".to-build");
  if (toBuild.get$length() == (0)) {
    return false;
  }
  toBuild.$index((0)).get$classes().remove("to-build");
  return true;
}
introClientServerSlides.prototype.nextSlide = function() {
...
introClientServerSlides.prototype.prevSlide = function() {
  if (this.curSlide > (0)) {
    this.curSlide--;
    this.updateSlides();
  }
}
introClientServerSlides.prototype.isChromeVoxActive = function() {
  return false;
}
introClientServerSlides.prototype.speakNextItem = function() {

}
introClientServerSlides.prototype.speakPrevItem = function() {

}
introClientServerSlides.prototype.addEventListeners = function() {
  var $this = this; // closure support
...
introClientServerSlides.prototype.makeBuildLists = function() {
  for (var i = this.curSlide, slide;
   $lt(i, this.slideEls.get$length()); i = $add(i, (1))) {
    slide = this.slideEls.$index(i);
    var items = slide.queryAll(".build > *");
    for (var j = (0), item = null;
     $lt(j, items.get$length()); j = $add(j, (1))) {
      item = items.$index(j);
      if (item.get$classes() != null) {
        item.get$classes().add("to-build");
      }
    }
  }
}
introClientServerSlides.prototype.addPrettify = function() {
  var els = html_get$document().queryAll("pre");
  for (var i = (0), el;
   $lt(i, els.get$length()); i = $add(i, (1))) {
    el = els.$index(i);
    if (!el.get$classes().contains("noprettyprint")) {
      el.get$classes().add("prettyprint");
    }
  }
...
introClientServerSlides.prototype.run = function() {
...
(function(){
  ...
  var v14/*EventTarget*/ = [v8/*AbstractWorker*/,v9/*Node*/,'EventTarget|DOMApplicationCache|DOMWindow|EventSource|MessagePort|Notification|SVGElementInstance|WebSocket|XMLHttpRequest|XMLHttpRequestUpload'].join('|');
